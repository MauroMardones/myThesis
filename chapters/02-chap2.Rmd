...
```{r length, out.width = "70%", fig.align='center', fig.cap="Length structures of krill from fishery dependent data. Red line represent recruitment size in 3.6 cm"}
...
ft_env <- flextable(env_features) |>
  ...
  set_caption("Summary of environmental variables used in the correlation analysis") |>
...
> The recruitment index exhibited clear spatial variability across the Management Units (MUs) of Subarea 48.1. In Bransfield Strait (BS) and the “Extra” MU, recruitment anomalies were predominantly positive, particularly after 2010, indicating favorable conditions for krill recruitment. By contrast, Elephant Island (EI) and South Shetland Inner Waters (SSIW) showed mostly negative anomalies, with only occasional positive signals, suggesting more constrained recruitment dynamics in those regions. The Joinville MU displayed very limited data, precluding robust interpretation. These spatially structured recruitment patterns are consistent with the influence of local environmental variability, as our analyses revealed significant correlations between recruitment indices and key environmental drivers.
