...
```{r conceptual, out.width='40%', fig.cap="Conceptual model used to model dynamics population in Antarctic krill in WAP"}
...
```{r rmseplot, fig.pos="H", out.width="75%", fig.cap="Distributions of Root Mean Square Error (RMSE) values for each scenario of the krill population model"}
...
The integration of penguin predation data into natural mortality estimates represents a novel approach in an integrated stock assessment, and even more so for krill. This size-selective predation pattern is biologically consistent with optimal foraging theory and observed predator behavior, where smaller krill are more vulnerable to capture but larger individuals remain accessible to key predators like *Pygoscelis* penguins [@Panasiuk2024]. Regarding scenarios of modeling, the scenarios with explicit inclusion of predator components (`s1.2` and `s1.4`), showed increased bias in estimates of fishing mortality (F) and spawning stock biomass (SSB), they resulted in lower estimates of recruitment, total biomass, and spawning biomass, offering a more precautionary and arguably more realistic representation of krill dynamics.
...
Incorporating predation mortality into stock assessment models is essential for accurately representing population dynamics in marine ecosystems. @Hollowed2000  revealed higher total natural mortality rates than traditional life-history estimates and showed that ignoring predation uncertainty underestimated stock biomass uncertainty by up to 20%. Similarly, @Richards2016 introduced a predation pressure index for northern shrimp, combining predator biomass and diet frequency to scale natural mortality over time. This method improved model fit and reduced retrospective bias without requiring absolute predator consumption data, which are often unavailable. For Antarctic krill stock assessments, incorporating predation variability improves mortality estimates, enhances biomass predictions, and supports ecosystem-based management. 

### Model Performance and Diagnostic Evaluation

The comprehensive diagnostic evaluation revealed important trade-offs between model realism and statistical performance. While the ecosystem-informed models (particularly `s1.4`) showed improved biological realism through reduced Pearson residuals and better representation of length-frequency distributions, they also exhibited increased retrospective bias and reduced likelihood values compared to the baseline model (`s1.1`). This pattern reflects a common challenge in ecosystem-based stock assessment: the tension between incorporating ecological complexity and maintaining statistical robustness (REF).

@Dong2025

The elevated retrospective bias in fishing mortality estimates for ecosystem scenarios, particularly the pronounced bias (rho = 6.912) in scenario `s1.4`, suggests potential overfitting or structural instability when both environmental and predator variables are simultaneously incorporated. This finding highlights the need for careful model validation and sensitivity analysis when developing ecosystem-informed assessment frameworks (REF). The observed bias patterns may also reflect temporal mismatches between ecosystem variables and population responses, suggesting that lagged effects or non-linear relationships should be explored in future model development.

### Implications for Precautionary Management

The consistently lower biomass and recruitment estimates produced by ecosystem-informed models have important implications for precautionary fisheries management. The significant reduction in spawning biomass estimates between scenarios `s1.1` and `s1.4` (mean difference: 2.6 million tonnes) demonstrates that neglecting ecosystem variables may lead to overly optimistic stock assessments and potentially unsustainable harvest recommendations. This finding aligns with CCAMLR's precautionary approach and supports the development of management strategies that explicitly account for ecosystem variability (REF).

The more conservative estimates produced by ecosystem-informed models provide a buffer against uncertainty in population status, which is particularly important given the high variability and unpredictability of Antarctic marine ecosystems (REF). However, the increased model complexity and associated statistical challenges must be balanced against the improved biological realism to ensure that management advice remains scientifically credible and operationally feasible.

### Spatial Considerations and the Areas-as-Fleets Approach

The implementation of the areas-as-fleets methodology successfully captured spatial heterogeneity in krill population dynamics across the five Management Units of Subarea 48.1. The spatial patterns observed in model residuals, particularly the consistent misfit in Bransfield Strait surveys, highlight the importance of considering spatial variability in selectivity and abundance patterns (REF). These spatial differences likely reflect distinct environmental conditions, predator assemblages, and krill population characteristics across the Western Antarctic Peninsula region (REF).

The identification of persistent residual patterns in specific spatial strata suggests that additional spatial considerations may be necessary to fully capture krill population dynamics. Future model development should consider incorporating explicit movement patterns, spatially-varying growth parameters, or environmental gradients that may influence local population processes (REF).

### Model Limitations and Future Directions

Several limitations of the current modeling approach warrant discussion. First, the integration of predator effects relied on relatively coarse-scale abundance indices that may not fully capture fine-scale predator-prey interactions or behavioral responses. More detailed predator foraging data, including spatial overlap indices and prey switching behavior, could enhance the representation of predation mortality (REF).

Second, the environmental covariate framework, while effective in capturing chlorophyll-driven recruitment variability, represents a simplified view of the complex environmental drivers affecting krill populations. Future studies should consider multi-variate environmental relationships, threshold effects, and lagged responses that may better represent the mechanistic links between climate variability and krill population dynamics (REF).

Third, the current model assumes static relationships between ecosystem variables and population processes. However, climate change may alter these relationships over time, requiring adaptive modeling approaches that can accommodate non-stationary ecosystem conditions (REF). The development of time-varying parameter frameworks or regime-specific models may be necessary to address this challenge.
